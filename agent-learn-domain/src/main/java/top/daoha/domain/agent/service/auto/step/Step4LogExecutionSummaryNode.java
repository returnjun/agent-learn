...
public class Step4LogExecutionSummaryNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        logExecutionSummary(
                ...
                Math.max(0, dynamicContext.getStep() - 1),
     ...
     * 记录执行总结
     */
    private void logExecutionSummary(int maxSteps, int executedSteps, boolean isCompleted) {
        log.info("\n📊 === 动态多轮执行总结 ====");

        int actualSteps = Math.min(maxSteps, executedSteps);
        ...
        // 计算执行效率
        double efficiency = isCompleted
                ? 100.0
                : maxSteps > 0 ? (double) actualSteps / maxSteps * 100 : 0.0;
        log.info("📊 执行效率: {}%", String.format(Locale.ROOT, "%.1f", efficiency));
    }
