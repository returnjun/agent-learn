...
import top.daoha.test.ai.advisors.RagAnswerAdvisor;
...
//                .defaultToolCallbacks(new SyncMcpToolCallbackProvider(stdioMcpClient(), sseMcpClient01(), sseMcpClient02()).getToolCallbacks())
    ...
    public void test_chat_model_stream_01() throws InterruptedException {
        ...
        Prompt prompt = Prompt.builder()
                .messages(new UserMessage("有哪些工具可以使用"))
                .build();

        Flux<ChatResponse> stream = chatModel.stream(prompt);

        stream
                // 无论正常结束还是发生异常，doFinally 都会执行，确保一定释放 Latch
                ...
                .subscribe(
                        chatResponse -> {
                            ...
                            if (chatResponse != null && chatResponse.getResult() != null) {
                                ...
                                if (output != null) {
                                    log.info("测试结果块: {}", JSON.toJSONString(output));
                                }
                            }
                        },
                        error -> {
                            log.error("流式输出发生异常: ", error);
                        }
                        // 第三个参数 onComplete 不需要再写 countDown，doFinally 已经处理了
                );

        // 增加超时时间，防止意外情况下永久挂起 (比如等 3 分钟)
        countDownLatch.await(3, java.util.concurrent.TimeUnit.MINUTES);
    }
...
//                .defaultTools(new SyncMcpToolCallbackProvider(sseMcpClient01(), sseMcpClient02()))
    ...
    public McpSyncClient stdioMcpClient() {
        ...
        String command = isWindows ? "npx.cmd" : "npx";

        var stdioParams = ServerParameters.builder(command)
                .args("-y", "@modelcontextprotocol/server-filesystem", "C:\\Users\\24209\\Desktop", "C:\\Users\\24209\\Desktop")
                .build();
